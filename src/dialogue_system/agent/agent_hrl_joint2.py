...
class AgentHRL_joint2(object):
    ...
    def next(self, state, turn, greedy_strategy, **kwargs):
        ...
        #state_rep = state_rep + state['wrong_disease_rep'].reshape(state_rep.shape)
        
        state_rep = np.append(state_rep, state['wrong_disease_rep'])
    ...
    def train_dqn(self):
        ...
        if self.parameter.get("prioritized_replay"):
            ...
            print("Batch - [Master agent] cur bellman err %.4f, experience replay pool %s" % (
    ...
    def record_training_sample(self, state, agent_action, reward, next_state, episode_over, lower_reward, master_action_index):
        ...
        state_rep = np.append(state_rep, state['wrong_disease_rep'])
        next_state_rep = np.append(next_state_rep, next_state['wrong_disease_rep'])
        master_state_rep = np.append(master_state_rep, self.master_state['wrong_disease_rep'])
