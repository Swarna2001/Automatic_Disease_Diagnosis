...
class Agent(object):
    ...
    def __init__(self, action_set, slot_set, disease_symptom, parameter, disease_as_action=True):
        ...
        # Contains the unique set of symptoms associated with all
        # the diseases in the disease_symptom dict
        symptom_set = set()
        
        # key = name of the disease
        # v = dictionary containing a "index" key and "symptom" key
        # v['symptom'].keys() = returns the name of all symptoms associated with the disease
        ...
        # This function will return a dict similar to the disease_symptom, except that
        # the symptom key would now be a list containing the names of the top symptoms
    ...
    def _build_action_space(self, disease_symptom, disease_as_action):
        ...
        for disease, v in disease_symptom.items():
            # list(v['symptom']) returns the name of the symptoms associated with the disease
            slot_set = slot_set + list(v["symptom"])

        # Eliminating the repeated set of symptoms from slot_set
    ...
    def disease_symptom_clip(disease_symptom, denominator, parameter):
        ...
        for key, value in disease_symptom.items():
            ...
            # sorts the symptom - frequency pair in descending order
            symptom_list = sorted(value['symptom'].items(), key=lambda x: x[1], reverse=True)
            
            # removing the frequencies and storing only the names of the symptoms
            symptom_list = [v[0] for v in symptom_list]
            
            # Extract the top relevant symptoms based on the arguments
            symptom_list = symptom_list[0:min(len(symptom_list), int(max_turn / float(denominator)))]
            
            # Instead of now having symptoms-frequency pairs, we now store the most relevant symptoms alone
