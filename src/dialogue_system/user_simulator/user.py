...
class User(object):
    def __init__(self, goal_set, disease_symptom, parameter):
        ...
        # Prepares the goal sets for the given User object
        self.goal_set, self.disease_sample_count = self.__prepare_goal_set__(goal_set,parameter)
        
        self.max_turn = parameter["max_turn"]
        self.parameter = parameter
        
        # Here the disease_symptom will be storing only the most relevenat symptoms
    ...
    def __prepare_goal_set__(self, goal_set, parameter):
        ...
        goal_set : dict
            ...
            A dictionary containing 3 keys namely train, test, validation.
            Each of these in turn hold a dictionary of user goals. A user goal is a 
            dictionary with the keys as consult_id, disease_tag, group_id and a 
            goal, which is a dictionary of all the types of slots
            
        
        parameter : argParser object, having 63 attributes
            
            Refer the spreadsheet to know its contents

        Returns
        -------
        temp_goal_set : dict
            
            A dictionary similar to the goal_set argument, containing only 
            those user goals that have the explicit and implicit number of 
            symptoms greater than a threshold value.
            
        disease_sample_count : dict
            
            A dictionary holding for each disease tag, the number of user goals
            associated with it, in the temp_goal_set dict.

        '''
        
        # Get the threshold values for explicit and implicit number 
        # of symptoms, as defined by us
        ...
        # Here key refers to whether its train, test or validation
        for key in goal_set.keys():
            
            # Create an empty list to hold the user goals
            temp_goal_set[key] = []
            
            for goal in goal_set[key]:
                
                # Flag variable to check if the current goal is added or not
                append_or_not = False
                
                # If the number of explicit and implicit symptoms in the user
                # goal is greater than our threshold, we set the flag to True
                if len(goal["goal"]["explicit_inform_slots"].keys()) >= explicit_number and \
                        len(goal["goal"]["implicit_inform_slots"].keys()) >= implicit_number:
                    append_or_not = True

                # If flag is true, we then add it into the temp_goal_set
                if append_or_not:
                    temp_goal_set[key].append(goal)
                    
                    # If the key doesn't exist in disease_sample_count,
                    # we create it and initialise it with zero
